...
def build_pipeline_from_pdf(pdf_file_path: str) -> dict:
    """Build the complete RAG pipeline from uploaded PDF file path."""
    if pdf_file_path is None:
        return None

    try:
        # 1) Load PDF directly from the uploaded file path
        print(f"Loading and cleaning PDF from: {pdf_file_path}")
        docs = load_pdf_from_file(pdf_file_path)
...
def upload_pdf(pdf_file):
    ...
    try:
        ...
        # Build pipeline directly from the uploaded file path
        global_pipeline = build_pipeline_from_pdf(pdf_file.name)
        ...
        sample_content = ""
        if global_pipeline["chunks"]:
            first_chunk = global_pipeline["chunks"][0].page_content[:200]
            sample_content = f"\n\n📖 Sample content:\n{first_chunk}..."
        
        success_msg = f"✅ PDF processed successfully!\n📄 Pages: {num_pages}\n📝 Chunks: {num_chunks}{sample_content}\n\nYou can now ask questions about the document."
